      ...
      onSuccess: (data)=> {
        ...
        try {
          ...
          Object.keys(data).forEach(date => {
              ...
              const dailySalary = dailyHours * 10;
    ...
    const handleDownload = async () => {
        if (selectedPeriod) {
            try {
                ...
                const { from, to } = monthToFromToDate(selectedPeriod);
                const response = fetch(`/api/user/pdfsummary?from=${from}&to=${to}`);
                const summary = await response.json();
                console.log('Work Summary:', summary);
                ...
                Object.keys(summary).forEach(date => {
                    const dailyHours = summary[date].totalHoursWorked;
                    const dailySalary = dailyHours * 10;
                    totalSalary += dailySalary;
